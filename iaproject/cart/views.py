...
def add(request, pk):
    ...
    if download not in cart.downloads.all():
...
def success(request):
    ...
    Gets Stripe's session id from url and
    uses to retrieve Stripe's session data.
    Gets current cart id from session.
